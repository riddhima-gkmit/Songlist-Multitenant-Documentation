...
Purpose, access rules, data scope, and business rules per module.  
Endpoints: [API Design](api-design.md).
...
- Own (LISTENER)
- Tenant (ADMIN)
- Platform count / admin list (SUPER_ADMIN)
...
- Global (platform-level resource)
...
**Purpose:** Song catalog with GLOBAL vs TENANT visibility.  
GLOBAL = platform; TENANT = tenant-specific.
...
- Release year validated (minimum year, not in future).
...
- `is_active` flags link state.
- Soft delete on links.
...
- Tenant (via playlist owner’s tenant)
...
**Purpose:** Razorpay premium subscriptions (lifetime).  
Payment links, webhooks, subscription status.
...
- Tenant (links, subscription)
- Platform (SUPER_ADMIN lists)
